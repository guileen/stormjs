  ...
  function gen_block(statments, in_async_fun, name_to_parent_asyncs) {
    ...
    function _for(ast) {
      ...
      if (next_body.depend_asyncs.length > 0) {
        is_serial = true;
        ret = new_node(make_serial_loop_ast(init, condition, step, body));
      } else {
...
function make_serial_loop_ast(init_vars, condition, step, block_node, after) {
  // vars
  var init_defs = [];
  var init_values = [];
  var next_args = [];
  ...
  var body = block_node.result.slice();
  ...
  if (after) {
    var after_ast;
    if (use_after_fn = after.result.length > 2) {
      init_defs.push('after');
      init_values.push(['function', , [], after.result]);
      after_ast = ['call', ['name', 'after'], []];
    } else {
      after_ast = ['block', after.result];
    }
    if_ast.push(after_ast);
  }
