...
function stormfy(ast, options) {
  ...
  function isAsyncCall(name) {
    return name && name.toString()[name.length - 1] === async_mark;
  }

  function gen_function(name, args, body, name_to_parent_asyncs) {
    var index = args.indexOf(callback_mark),
        is_async = index >= 0 ||
        (isAsyncCall(name) && name = name.substring(0, name.length - 1));
    ...
    function _if(ast) {
      ...
      if (is_async) {
        ...
        ret.result = [ast[0], condition.result, ['block', then_node.toAST()],
          else_ast && ['block', else_node.toAST()]];
      }
      join_node(ret, condition);
    ...
    function gen_node(ast, options) {
      ...
      case 'name':
        ...
        if (options && options.calling && isAsyncCall(ast[1])) {
          ...
          ast[1] = ast[1].substring(0, ast[1].length - 1);
        }
        ret.result = ast;
        if (!is_sub) {
      ...
      case 'dot':
        ...
        if (options && options.calling && isAsyncCall(ast[2])) {
          ret.is_async_caller = true;
          ast[2] = ast[2].substring(0, ast[2].length - 1);
        }
