...
function stormfy(ast, options) {
  ...
  var callback_name = options.callback_name || 'callback';
  var async_mark = keywords.async_mark || '$';
  var callback_mark = keywords.callback_mark || '$';
  var async_regex = new RegExp(regExpSafe(async_mark) + '$');
  ...
  function isAsyncCall(name) {
    return async_regex.test(name);
  }

  function normalizeAsyncCall(name) {
    return name.replace(async_regex, '');
  }

  function gen_function(name, args, body, name_to_parent_asyncs) {
    var index = args.indexOf(callback_mark),
        is_async = index >= 0;

    if (isAsyncCall(name)) {
      is_async = true;
      name = normalizeAsyncCall(name);
    }

    if (is_async) {
      args[index >= 0 ? index : args.length] = callback_name;
    }

    var type = this[0];
    var block_node = gen_block(body, is_async, name_to_parent_asyncs);
    var ret = new_node([type, name, args, block_node.toAST()]);
    ret.is_async_caller = is_async;
   ...
   */
  function make_async_call(fn, args, options) {
    var body = [
      ['if',
        ['name', '_err'],
        ['return',
          ['call', ['name', callback_name], [['name', '_err']]]]]];
    var left_name = options && options.left_name;
    var index = indexOfName(args, callback_mark);
    index = index >= 0 ? index : args.length;
    var reply_name = '_' + (left_name ? left_name.replace(/\./g, '_') : ++reply_count);
    args[index] = ['function', '', ['_err', reply_name], body];
    return {
      is_async: true,
      ast: ['call', fn, args],
      callback: args[index],
      callback_args: args[index][2],
  ...
  function gen_block(statments, in_async_fun, name_to_parent_asyncs) {
    ...
    var node = new_node('NORESULT_BLOCK', function(end_ast) {
        return make_block_ast(nodes, true, end_ast);
    });
    ...
    function _set_var_depend_asyncs(ast, asyncs, map) {
      ...
      case 'name':
        return map[ast[1]] = {'*' : asyncs.slice()};
      case 'dot':
        map = _add_var_depend_asyncs(ast[1], asyncs, map);
        return map && _set_var_depend_asyncs(ast[2], asyncs, map);
      case 'sub':
        //return a['x'] return a.x
        map = _add_var_depend_asyncs(ast[1], asyncs, map);
        if (map && ast[2][0] == 'string') {
    ...
    function _add_var_depend_asyncs(ast, asyncs, map) {
      ...
      case 'dot':
        map = _add_var_depend_asyncs(ast[1], asyncs, map);
        return map && _add_var_depend_asyncs(ast[2], asyncs, map);
      case 'sub':
        //return a['x'] return a.x
        map = _add_var_depend_asyncs(ast[1], asyncs, map);
        if (map && ast[2][0] == 'string') {
    ...
    function _var(ast) {
      ...
      for (var j = 0; j < defs.length; j++) {
        ...
        locals.push(def[0]);
        if (def.length > 1) {
          var def1 = gen_node(def[1], {left_name: def[0]});
    ...
    function _call(ast, options) {
      var i = 0, args = ast[2], rargs = [], self_is_async, ret;
      for (; i < args.length; i++) {
        var arg = args[i];
        if (arg[0] == 'name' && arg[1] == callback_mark) {
          self_is_async = true;
      ...
      var func = gen_node(ast[1], {calling: true});
      self_is_async || (self_is_async = func.is_async_caller);
      if (self_is_async) {
        // process all asyncs, and make ret
        ret = make_async_call(func.result, rargs.map(function(rargs) {return rargs.result}), options);
      }else {
    ...
    function _if(ast) {
      var condition = gen_node(ast[1]),
          then_ast = ast[2],
          else_ast = ast[3],
          then_node, else_node, ret;
      ...
      if (!else_ast) {
        else_ast = [];
      } else if (else_ast[0] == 'block') {
        else_ast = else_ast[1];
      } else {
        else_ast = [else_ast];
      }

      then_node = gen_block(then_ast, in_async_fun, name_depend_asyncs);
      else_node = gen_block(else_ast, in_async_fun, name_depend_asyncs);

      // update var depend asyncs map
      var is_async = false, ret = new_node();
      for (var name in then_node.global_depend_inner_asyncs) {
        set_var_depend_asyncs(['name', name], [ret]);
        is_async = true;
      }
      for (var name in else_node.global_depend_inner_asyncs) {
        set_var_depend_asyncs(['name', name], [ret]);
        is_async = true;
      }

      if (is_async) {
        ret.result = 'NORESULT-IF';
        ret.toAST = function() {
          return make_if_ast(condition, then_node, else_node, this.nodes);
        };
        ret.is_async = true;
      } else {
        ret.result = [ast[0], condition.result, ['block', then_node.toAST()],
          else_ast.length > 0 && ['block', else_node.toAST()]];
      }
      join_node(ret, condition);
      addAllToSet(ret.depend_asyncs, then_node.depend_asyncs);
      addAllToSet(ret.depend_asyncs, else_node.depend_asyncs);
      return ret;
     ...
     */
    function gen_node(ast, options) {
      var type = ast[0], ret;
      var is_sub = options && options.is_sub;
      ...
      case 'assign':
        ...
        var right = gen_node(ast[3], {left_name: name});
        ret = new_node([type, ast[1], left.result, right.result]);
        join_node(ret, left);
        join_node(ret, right);
      ...
      case 'stat':
        ...
        break;
      case 'return':
        var right = gen_node(ast[1], {left_name: 'result'});
        if (in_async_fun) {
      ...
      case 'var':
        return _var(ast);
      case 'call':
        return _call(ast, options);
      case 'name':
        ret = new_node();
        if (options && options.calling && isAsyncCall(ast[1])) {
          ret.is_async_caller = true;
          ast[1] = normalizeAsyncCall(ast[1]);
        }
        ret.result = ast;
        if (!is_sub) {
      ...
      case 'dot':
        ret = new_node();
        if (options && options.calling && isAsyncCall(ast[2])) {
          ret.is_async_caller = true;
          ast[2] = normalizeAsyncCall(ast[2]);
        }
        var left = gen_node(ast[1], options);
        ret.result = [type, left.result, ast[2]];
        join_node(ret, left);
        if (!is_sub) {
          addAllToSet(ret.depend_asyncs, get_var_depend_asyncs(ast));
        }
...
function make_nodes_tree(nodes, top, end_ast) {
  ...
  if(end_ast){
    var end_node = new_node(end_ast);
    for (var i = 0; i < nodes.length; i++) {
      var node = nodes[i];
      if(node.is_async){
        end_node.depend_asyncs.push(node);
      }
    }
    nodes.push(end_node);
  }
...
function make_block_ast(nodes, toplevel, end_ast) {
  var bodys = [];
  var defs = [];
  ...
  var tree = make_nodes_tree(nodes, null, end_ast);
...
function make_serial_loop_ast(init_vars, condition, step, block_node, nodes) {
  ...
  var body = block_node.toAST(['stat', ['call', ['name', 'next'], next_args]]);
  ...
  if (nodes && nodes.length > 0) {
    var nodes_ast;
    if (use_nodes_fn = (nodes.length > 1 || nodes[0].is_async)) {
      init_defs.push('after');
      init_values.push(['function', , [], make_nodes_ast(nodes)]);
      nodes_ast = ['stat', ['call', ['name', 'after'], []]];
    } else {
      nodes_ast = ['block',
...
function make_if_ast(condition, then_node, else_node, nodes) {

  var init_values = [];
  var init_defs = [];
  var then_ast = then_node.toAST();
  var else_ast = else_node.toAST();

  var if_ast = ['if', condition.result, ['block', then_ast]];
  ...
  if (nodes && nodes.length > 0) {
    ...
    if (use_nodes_fn = (nodes.length > 1 || nodes[0].is_async)) {
      ...
      nodes_ast = nodes[0].toAST ? nodes[0].toAST() : nodes[0].result;
    }
    then_ast.push(nodes_ast);
    else_ast.push(nodes_ast);
    if_ast.push(['block', else_ast]);

    if (use_nodes_fn) {
      var fn_condition = ['function', '', init_defs, [if_ast]];
      return ['stat', ['call', fn_condition, init_values]];
    }
  } else {
    if (else_ast.length > 0) {
      if_ast.push(['block', else_ast]);
    }
  }
...
/**
 * merge another to obj, array value will push if not exists
 */
function merge(obj, another) {
  for (var i in another) {
    if (! obj[i]) {
      obj[i] = clone(another[i]);
    } else if (Array.isArray(another[i])) {
      addAllToSet(obj[i] || (obj[i] = []), another[i]);
    } else {
      merge(obj[i], another[i]);
    }
  }
  return obj;
}
