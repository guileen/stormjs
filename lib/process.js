...
function stormfy(ast, callback_name) {
  ...
  function gen_block(statments, in_async_fun, name_to_parent_asyncs) {
    ...
    function get_global_depend_inner_asyncs(){
      ...
      for(var name in name_depend_asyncs){
        if(locals.indexOf(name) < 0){
          var async = name_depend_asyncs[name];
    ...
    function _for(ast) {
      ...
      if (next_body.depend_asyncs >= 0) {
        is_serial = true;
      }

      var ret = new_node([ast[0], init.result, condition.result, step.result, ['block', body.result]]);
      join_node(ret, init);
...
function make_vars_ast(nodes) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    if (node.is_async && node.out_ref) {
      defs.push([result[1]]);
    } else if (result[0] == 'var' && node.depend_asyncs.length > 0) {
      var _defs = result[1];
      var _i = i;
