...
function stormfy(ast, callback_name) {
  ...
  function walk_block(statments, in_async_fun) {
    ...
    function gen_node(ast) {
      ...
      case 'name':
        ret = new_node(ast);
        ret.asyncs = ret.depend_asyncs = name_to_asyncs[ast[1]] || [];
        break;
      default:
        ret = new_node(ast);
      }
...
function make_parallel_body(asyncs, after_asyncs) {
  ...
  var parallel_vars = make_parallel_vars(asyncs);
  ...
  var bodys = [parallel_vars, make_parallel_fn_update(asyncs, after_asyncs)];
...
if (!module.parent) {
  ...
  function test() {
    ...
    var bar1 = foo(_) + 5 + foo(_);
    var bar2 = foo(foo1(_) + foo2(_), _);
    var bar3 = foo(_);
    var bar4 = foo(bar3, _);
    var bar5 = bar1 + bar2 + bar3 + bar4;
