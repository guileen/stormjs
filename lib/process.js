  ...
  function gen_block(statments, in_async_fun, name_to_parent_asyncs) {
    ...
    function _call(ast) {
      ...
      if (self_is_async) {
        ...
        var self_async = make_async_call(func.result, MAP(rargs, function(rargs) {return rargs.result}));
      ...
      if (self_is_async) {
        ...
        ret.depend_asyncs = [self_async]; // is and must only depends self_async
      }
      return ret;
    }
...
function make_block_ast(nodes, toplevel) {
  ...
  var with_trigger = multi_depends.length > 1;
  var with_update = multi_depends.length === 1;

  if (with_trigger) {
    bodys.push(make_triggers(multi_depends));
    bodys.push(make_fn_trigger(multi_depends));
  } else if(with_update) {
    bodys.push(make_update_vars_ast(nodes, multi_depends));
...
function make_triggers(multi_depends) {
  var groups = group_multi_depends(multi_depends),
      triggers = [];
...
function make_update_ast(multi_depends) {
  ...
  for (var i = 0; i < multi_depends.length; i++) {
    block.push(multi_depends[i].result);
  }
