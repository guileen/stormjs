...
function stormfy(ast, callback_name) {
  ...
  function walk_block(statments, in_async_fun) {
    ...
    return make_nodes_ast(nodes, true);
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    if (len === 0) {
      ...
      for (var j = 0; j < node.depend_asyncs.length; j++) {
        var dep_roots = node.depend_asyncs[j].roots;
        if (dep_roots.length < 2) {
          // not in after_asyncs, not in same scope
...
function make_nodes_ast(nodes, toplevel) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    var node = nodes[i];
    if (node.is_async) {
      bodys.push(make_async_ast(node, after_asyncs.length > 0));
    } else {
...
function make_async_ast(async, with_update) {
  if (async.out_ref) {
