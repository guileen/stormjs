  ...
  function walk_block(statments, in_async_fun) {
    ...
    function gen_node(ast) {
      ...
      case 'assign':
        var left = gen_node(ast[2]);
        var right = gen_node(ast[3]);
        var ret = new_node([type, ast[1], left.result, right.result]);
        join_node(ret, left);
        join_node(ret, right);
      ...
      case 'stat':
        ...
        var ret = new_node([type, stat.result]);
        sync_node(ret, stat);
        break;
      case 'return':
      ...
      case 'dot':
        ...
        var ret = new_node([type, left.result, ast[2]]);
        join_node(ret, left);
        if(name){
      ...
      case 'sub':
        ...
        var ret = new_node([type, left.result, right.result]);
        join_node(ret, left);
        if(name){
          addToSet(ret.depend_asyncs, name_depend_asyncs[name] || []);
        }
